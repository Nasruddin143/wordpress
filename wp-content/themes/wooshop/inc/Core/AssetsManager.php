...
final class AssetsManager
{
    ...
    private function process_style(
        ...
        wp_enqueue_style(
            $handle,
            $src,
            $this->normalize_dependencies($asset['deps'] ?? []),
     ...
     * @return void
     */
    private function process_script(
        string $handle,
        array $asset
    ): void {
        if (!$this->should_load($asset)) {
    ...
    private function should_load(array $asset): bool
    {
        ...
        if (is_array($condition)) {
            foreach ($condition as $condition_name) {
                if (
                    is_string($condition_name) &&
                    $this->check_condition($condition_name)
                ) {
                    return true;
                }
            }
